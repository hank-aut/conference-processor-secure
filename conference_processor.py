            ...
            for classification, attendees in results.items():
                ...
                if attendees:
                    ...
                    for attendee in attendees:
                        ...
                        # Add Relationship Owner column for current customers
                        if classification == 'current_customers' and 'relationship_owner' in attendee:
                            record['Relationship Owner'] = attendee.get('relationship_owner', '')
                        
                        # Add Opportunity Owner column for open opportunities
                        if classification == 'open_opportunities' and 'opportunity_owner' in attendee:
                    ...
                    if classification == 'excluded':
                        ...
                        df = pd.DataFrame(columns=['First Name', 'Last Name', 'Company', 'Title', 'Email', 'Relationship Owner'])
                    elif classification == 'open_opportunities':
                        df = pd.DataFrame(columns=['First Name', 'Last Name', 'Company', 'Title', 'Email', 'Opportunity Owner'])
                    else:
...
class SalesforceClassifier:
    ...
    def check_open_opportunities(self, account_id: str) -> Tuple[int, Optional[str]]:
        """Check for open opportunities on an account and return owner name
        Returns: (count, opportunity_owner_name)
        """
        if not self.sf:
            return 0, None
    ...
    def classify_attendee(self, attendee: Dict, email: Optional[str]) -> Tuple[str, Dict]:
        ...
        if account_id:
            open_opps, opportunity_owner = self.check_open_opportunities(account_id)
            if open_opps > 0:
