...
class EmailDiscovery:
    ...
    def call_apollo_api_direct(self, first_name: str, last_name: str, company: str) -> Tuple[Optional[str], str, Optional[str]]:
        ...
        if not api_key:
            return None, None, "Apollo API: API key not found in environment variables"
...
'''
    ...
    def apollo_company_pattern_search(self, first_name: str, last_name: str, company: str) -> Tuple[Optional[str], str]:
        ...
        try:
            ...
            if not api_key:
                return None, "Apollo API: API key not found in environment variables"
...
class SalesforceClassifier:
    ...
    def _connect_to_salesforce(self):
        ...
        try:
            ...
            if not all([username, password, security_token]):
                print("    Salesforce: Missing required environment variables (SALESFORCE_USERNAME, SALESFORCE_PASSWORD, SALESFORCE_TOKEN)")
                self.sf = None
                return
    ...
    def search_by_email(self, email: str) -> Optional[Dict]:
        ...
        try:
            ...
            contact_query = f"SELECT Id, Name, Email, AccountId, Account.Name, Account.Customer_Designation__c, LastActivityDate, SystemModstamp FROM Contact WHERE Email = '{email}' LIMIT 1"
    ...
    def get_account_details(self, account_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            soql_query = f"SELECT Id, Name, Customer_Designation__c, Website, LastActivityDate, SystemModstamp FROM Account WHERE Id = '{account_id}'"
    ...
    def search_by_domain(self, email: str) -> Optional[Dict]:
        ...
        try:
            ...
            soql_query = f"SELECT Id, Name, Website, Customer_Designation__c, LastActivityDate, SystemModstamp FROM Account WHERE Website LIKE '%{domain}%'"
    ...
    def search_by_company(self, company: str) -> Optional[Dict]:
        ...
        for variation in variations:
            try:
                ...
                sosl_query = f"FIND {{{escaped_variation}}} IN NAME FIELDS RETURNING Account(Id, Name, Website, Customer_Designation__c, LastActivityDate, SystemModstamp)"
    ...
    def check_open_opportunities(self, account_id: str) -> int:
        ...
        try:
            ...
            soql_query = f"SELECT COUNT() FROM Opportunity WHERE AccountId = '{account_id}' AND IsClosed = false"
    ...
    def classify_attendee(self, attendee: Dict, email: Optional[str]) -> Tuple[str, Dict]:
        ...
        if company in self.company_classifications:
            cached_classification, cached_reason = self.company_classifications[company]
            details['classification_reason'] = f"Company-level cached: {cached_reason}"
        ...
        if not sf_match:
            ...
            self.company_classifications[company] = (classification, reason)
        ...
        if account_id:
            open_opps = self.check_open_opportunities(account_id)
            if open_opps > 0:
...
class ConferenceProcessor:
    ...
    def process_file(self, csv_file_path: str):
        ...
        # Process first 10 attendees only for testing
        attendees_to_process = attendees[:10]
        print(f"Processing first 10 attendees only: {len(attendees_to_process)} attendees")
        
        # Process each attendee
        for i, attendee in enumerate(attendees_to_process):
            print(f"\nProcessing {i+1}/10: {attendee['first_name']} {attendee['last_name']} ({attendee['company']})")
            
            # Update progress
            progress['current_attendee'] = attendee
            progress['processed_count'] = i
            progress['phase'] = 'email_discovery'
            progress['total_attendees'] = 5  # Update to show only 5
            self.progress_tracker.save_progress(progress)
            ...
            # Store result - add reason for excluded attendees
            if classification == 'excluded':
                # Add reason to excluded attendee
                attendee_with_reason = attendee.copy()
                attendee_with_reason['reason'] = details.get('classification_reason', 'Unknown reason')
                results[classification].append(attendee_with_reason)
            else:
