...
class SalesforceClassifier:
    ...
    def check_open_opportunities(self, account_id: str) -> Tuple[int, Optional[str], Optional[str], Optional[str]]:
        ...
        try:
            ...
            if result.get('records') and len(result['records']) > 0:
                ...
                print(f"    DEBUG: Open Opportunity extraction - Found Opp ID: {opportunity_id}, Owner: {owner_name}")
                
                # Create Salesforce URL for the opportunity - MUST have valid ID
                if opportunity_id:
                    opportunity_url = f"https://datacenterhawk.lightning.force.com/lightning/r/Opportunity/{opportunity_id}/view"
                    print(f"    SUCCESS: Opportunity URL created: {opportunity_url}")
                else:
                    print(f"    ERROR: No Opportunity ID found in record: {first_opp}")
                    count = 0  # If we can't get the ID, don't count it as an open opportunity
            else:
                print(f"    ERROR: Count shows {count} opportunities but no records returned from query")
                count = 0  # If no records, reset count to 0
            
            # Final validation - if we say there are opportunities, we MUST have an ID and URL
            if count > 0 and (not opportunity_id or not opportunity_url):
                print(f"    WARNING: Resetting count to 0 because we couldn't extract valid Opportunity ID/URL")
                count = 0
                opportunity_id = None
                opportunity_url = None
                owner_name = None
    ...
    def classify_attendee(self, attendee: Dict, email: Optional[str]) -> Tuple[str, Dict]:
        ...
        print(f"    DEBUG: Customer_Designation__c = '{customer_designation}' for {sf_match.get('name', 'Unknown')}")
        
        if customer_designation == 'Current Customer':
            classification = 'current_customers'
            reason = f"Company '{company}' is a current customer - SF Customer_Designation__c = '{customer_designation}' - Matched: {match_info}"
            details['classification_reason'] = reason
            details['matched_record'] = match_info
            details['account_owner'] = sf_match.get('account_owner')
            details['salesforce_citation'] = {
                'record_id': sf_match.get('id'),
                'record_name': sf_match.get('name'),
                'record_type': sf_match.get('type'),
                'customer_designation_field': customer_designation,
                'source': 'Customer_Designation__c field in Salesforce'
            }
            
            # Add Account ID and URL for current customers - MUST have valid Account ID
            account_id = None
            if sf_match['type'] == 'Account':
                account_id = sf_match.get('id')
            elif sf_match['type'] in ['Contact', 'Lead']:
                account_id = sf_match.get('account_id')
            
            print(f"    DEBUG: Current Customer Account ID extraction - Type: {sf_match['type']}, ID: {sf_match.get('id')}, Account ID: {sf_match.get('account_id')} -> Final: {account_id}")
            
            if account_id:
                details['account_id'] = account_id
                details['account_url'] = f"https://datacenterhawk.lightning.force.com/lightning/r/Account/{account_id}/view"
                print(f"    SUCCESS: Current Customer Account URL: {details['account_url']}")
            else:
                print(f"    ERROR: No Account ID found for Current Customer {company} - Type: {sf_match['type']}, Match: {sf_match}")
                # Force account_id to the record ID if it's the best we have
                fallback_id = sf_match.get('id')
                if fallback_id:
                    details['account_id'] = fallback_id  
                    details['account_url'] = f"https://datacenterhawk.lightning.force.com/lightning/r/{sf_match['type']}/{fallback_id}/view"
                    print(f"    FALLBACK: Using {sf_match['type']} record URL: {details['account_url']}")
        ...
        if account_id:
            print(f"    DEBUG: Checking open opportunities for Account ID: {account_id}")
            open_opps, opportunity_owner, opportunity_id, opportunity_url = self.check_open_opportunities(account_id)
            print(f"    DEBUG: Found {open_opps} open opportunities")
            if open_opps > 0:
                classification = 'open_opportunities'
                reason = f"Company has {open_opps} open opportunities - Account ID: {account_id} - Matched: {match_info}"
                details['classification_reason'] = reason
                details['matched_record'] = match_info
                ...
                details['salesforce_citation'] = {
                    ...
                    'account_name': sf_match.get('name'),
                    'opportunity_count': open_opps,
                    'opportunity_id': opportunity_id,
                    'opportunity_owner': opportunity_owner,
                    'source': 'Salesforce Opportunity query: IsClosed = false'
                }
                self.company_classifications[company] = (classification, details.copy())
        ...
        print(f"    DEBUG: ROE Check - LastActivity: {last_activity}, SystemModstamp: {system_modstamp}")
        ...
        if last_activity or system_modstamp:
            ...
            details['salesforce_citation'] = {
                ...
                'record_type': sf_match.get('type'),
                'customer_designation_field': customer_designation,
                'roe_qualification': roe_reason,
                'source': 'ROE qualification based on LastActivityDate and SystemModstamp'
            }
...
class ConferenceProcessor:
    ...
    def process_file(self, csv_file_path: str):
        ...
        for i, attendee in enumerate(attendees_to_process):
            ...
            # FINAL VALIDATION: Ensure required IDs exist before adding to results
            if classification == 'current_customers' and not details.get('account_id'):
                print(f"    CRITICAL ERROR: {attendee['Full Name']} classified as Current Customer but no Account ID! Reclassifying as salesforce_qualified")
                classification = 'salesforce_qualified'
                attendee_with_details['account_id'] = None
                attendee_with_details['account_url'] = None
            elif classification == 'open_opportunities' and not details.get('opportunity_id'):
                print(f"    CRITICAL ERROR: {attendee['Full Name']} classified as Open Opportunity but no Opportunity ID! Reclassifying as salesforce_qualified")
                classification = 'salesforce_qualified'
                attendee_with_details['opportunity_owner'] = None
                attendee_with_details['opportunity_id'] = None
                attendee_with_details['opportunity_url'] = None
