        ...
        class StreamlitConferenceProcessor(ConferenceProcessor):
            ...
            def process_attendee_with_progress(self, attendee, index, total):
                ...
                if classification == 'excluded':
                    ...
                    attendee['account_id'] = details.get('account_id')
                    attendee['account_url'] = details.get('account_url')
                elif classification == 'open_opportunities':
                    attendee['opportunity_owner'] = details.get('opportunity_owner')
                    attendee['opportunity_id'] = details.get('opportunity_id')
                    attendee['opportunity_url'] = details.get('opportunity_url')
...
def generate_processing_log(processing_log):
    ...
    for entry in processing_log:
        ...
        if sf_class.get('details'):
            ...
            # Add Salesforce Citation for accountability
            if details.get('salesforce_citation'):
                citation = details['salesforce_citation']
                log_lines.append("   📋 SALESFORCE CITATION:")
                log_lines.append(f"      Source: {citation.get('source', 'Unknown')}")
                if citation.get('record_id'):
                    log_lines.append(f"      Record ID: {citation['record_id']}")
                if citation.get('record_name'):
                    log_lines.append(f"      Record Name: {citation['record_name']}")
                if citation.get('record_type'):
                    log_lines.append(f"      Record Type: {citation['record_type']}")
                if citation.get('customer_designation_field') is not None:
                    log_lines.append(f"      Customer_Designation__c: '{citation['customer_designation_field']}'")
                if citation.get('roe_qualification'):
                    log_lines.append(f"      ROE Qualification: {citation['roe_qualification']}")
                if citation.get('opportunity_count'):
                    log_lines.append(f"      Open Opportunity Count: {citation['opportunity_count']}")
                if citation.get('opportunity_id'):
                    log_lines.append(f"      Opportunity ID: {citation['opportunity_id']}")
                if citation.get('opportunity_owner'):
                    log_lines.append(f"      Opportunity Owner: {citation['opportunity_owner']}")
        log_lines.append("")
